 ...
 //to store values after calculating sum of each row for bar, changing original input is not advisable.
        ...
        for(int j = 0; j < m ; j++){ //O(M * N)
            ...
            for(int i = 0; i < n; i++){
                ...
                sum += matrix[i][j] == '1' ? 1 : 0 ; //cz matrix has characters not int
                if(matrix[i][j] == '0'){
                    sum = 0;
                }
                p_sum[i][j] = sum;
        ...
        for(int i = 0; i < n ; i++){ //O(N * 2M)
           ...
           Maxarea = max(Maxarea, lHist(p_sum[i]));  //lHist take O(2M) time as m is no. of col. and it calculates max rect for col. only
        }
        return Maxarea;
    }
